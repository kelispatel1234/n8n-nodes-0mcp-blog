# 0mcp Blog n8n community node

This Repository is for Admin to manage blogs
...
2. Add the **0mcp Blog** node to a workflow.
3. Select a resource and operation, configure its parameters, and execute the workflow.

## Example workflow

Connect **Manual Trigger** -> **0mcp Blog** -> a destination node, select an operation, then run the workflow and inspect the returned items.
...
`npm run dev` starts a local n8n development instance. Find the integration by its **0mcp Blog** display name.
